...
use crate::{
    alloc_proxy::proxy::Allocator, layout::AugmentedRBTreeLayout, node::internal_details::NodeRef,
    policy::internal_details::TreePolicy,
};

/// A guarded mutable reference to an augmented tree node.
///
...
#[derive(Debug)]
pub struct NodeGuard<'a, K, V, S, P>
where
...
impl<K, V, S, P> NodeGuard<'_, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    ...
    /// Returns a reference to the key of the guarded node.
    #[must_use]
    pub fn key(&self) -> &K {
        unsafe { self.node.key() }
    }

    /// Returns a reference to the statistics of the guarded node.
    #[must_use]
    pub fn stats(&self) -> &S {
        unsafe { self.node.stats() }
    }

    /// Returns a mutable reference to the value of the guarded node.
    #[must_use]
    pub fn value_mut(&mut self) -> &mut V {
        unsafe { self.node.value_mut() }
    }
    /// Returns a reference to the value of the guarded node.
    #[must_use]
    pub fn value(&self) -> &V {
...
impl<K, V, S, P> AsMut<V> for NodeGuard<'_, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
...
impl<K, V, S, P> Deref for NodeGuard<'_, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    type Target = V;
...
impl<K, V, S, P> DerefMut for NodeGuard<'_, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    #[inline]
...
impl<K, V, S, P> Drop for NodeGuard<'_, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    fn drop(&mut self) {
...
/// A guarded mutable reference to an augmented tree node value.
///
/// Modifying the underlying value via this guard automatically
/// propagates augmentation updates up the tree when it goes out of scope.
#[derive(Debug)]
pub struct ValueGuard<'a, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    node: NodeRef<K, V, S>,
...
impl<K, V, S, P> ValueGuard<'_, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    pub(crate) fn new(node: NodeRef<K, V, S>) -> Self {
...
impl<K, V, S, P> Deref for ValueGuard<'_, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    type Target = V;
...
impl<K, V, S, P> DerefMut for ValueGuard<'_, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    #[inline]
...
impl<K, V, S, P> Drop for ValueGuard<'_, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    fn drop(&mut self) {
...
///
/// This struct is created by the [`iter_mut`](crate::AugmentedRBTreeInt::values_mut) method.
#[derive(Debug)]
pub struct ValuesMut<'a, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    next: Option<NodeRef<K, V, S>>,
...
impl<K, V, S, P> ValuesMut<'_, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    pub(crate) fn new(root: Option<NodeRef<K, V, S>>, len: usize) -> Self {
...
impl<'a, K, V, S, P> Iterator for ValuesMut<'a, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    type Item = ValueGuard<'a, K, V, S, P>;
...
impl<'a, K, V, S: 'a, P> DoubleEndedIterator for ValuesMut<'a, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    fn next_back(&mut self) -> Option<Self::Item> {
...
impl<'a, K, V, S: 'a, P> ExactSizeIterator for ValuesMut<'a, K, V, S, P>
where
    P: TreePolicy<K = K, V = V, S = S>,
{
    fn len(&self) -> usize {
...
impl<'a, K, V, S: 'a, P> core::iter::FusedIterator for ValuesMut<'a, K, V, S, P> where
    P: TreePolicy<K = K, V = V, S = S>
{
}
